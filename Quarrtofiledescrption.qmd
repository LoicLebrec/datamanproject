...
#### **Dataset Highlights**
1. **Temporal Scope:**
   - The base year is 2012, with data extrapolated and now-casted up to 2021.
   - Updates include macroeconomic data (UN), trade (Comtrade/BACI), energy balances (IEA), and emissions (Edgar Database, PRIMAST).
...
- **How to Use**:
    - Use \( A \) to compute direct and indirect production dependencies.
    - Combine with \( x \) (total output) to calculate \( Z \) (inter-industry flows): \( Z = A \times x \).
...
- **How to Use**:
    - Analyze economic dependencies between sectors.
    - Combine with \( Y \) (final demand) to calculate total output: \( x = Z + Y \).
